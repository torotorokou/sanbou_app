# Database Development Tools

**目的**: 開発環境でのスキーマ管理・検証ツール

---

## 📌 このディレクトリについて

このディレクトリには、**開発環境専用**のデータベースツールが格納されています。

> **⚠️ 本番環境の権限管理について**  
> 本番環境のロール・権限管理は [ops/db/](../../ops/db/) を参照してください。  
> こちらは開発用のスキーマダンプ・検証ツールのみです。

---

## 🔧 利用可能なスクリプト

### `dump_schema_current.sh`

**目的**: 現在のスキーマ全体をSQLファイルにダンプ

```bash
# 使い方
bash scripts/db/dump_schema_current.sh

# または Makefile 経由
make db-dev-dump-schema ENV=local_dev
```

**出力先**: `app/backend/core_api/migrations/alembic/sql_current/schema_head.sql`

**用途**:
- スキーマ変更のレビュー
- マイグレーション前後の比較
- ドキュメント化

---

### `export_schema_baseline_local_dev.sh`

**目的**: Alembic v2 の起点となるスキーマベースラインをエクスポート

```bash
# 使い方
bash scripts/db/export_schema_baseline_local_dev.sh

# または Makefile 経由
make db-dev-export-baseline ENV=local_dev
```

**出力先**: `app/backend/core_api/migrations_v2/sql/schema_baseline.sql`

**前提条件**:
- local_dev 環境が起動済み (`make up ENV=local_dev`)
- Alembic が head まで適用済み (`make al-up ENV=local_dev`)

**用途**:
- Alembic マイグレーションの基準点作成
- 新しい環境へのスキーマ移行

---

## 🔗 関連リソース

### 本番環境運用

| 項目 | 場所 |
|------|------|
| **権限管理** | [ops/db/](../../ops/db/) |
| **ロール設計** | [ops/db/README.md](../../ops/db/README.md) |
| **実行用SQL** | [ops/db/sql/](../../ops/db/sql/) |
| **旧スクリプト** | [ops/db/legacy/](../../ops/db/legacy/) |

### Makefile コマンド

| コマンド | 説明 |
|---------|------|
| `make db-dev-dump-schema` | スキーマダンプ |
| `make db-dev-export-baseline` | ベースラインエクスポート |
| `make db-apply-all ENV=...` | 本番用ロール適用（ops/db/sql/使用） |

詳細は [mk/50_db_roles.mk](../../mk/50_db_roles.mk) を参照。

---

## 📚 ベストプラクティス

### スキーマダンプのタイミング

- ✅ **マイグレーション適用前**: 現状を記録
- ✅ **マイグレーション適用後**: 変更を確認
- ✅ **レビュー前**: PRに添付
- ✅ **リリース前**: 最終確認

### ファイル管理

```bash
# スキーマダンプは Git 管理下
git add app/backend/core_api/migrations/alembic/sql_current/schema_head.sql
git commit -m "docs: update schema dump after migration XXXX"

# ベースラインも Git 管理
git add app/backend/core_api/migrations_v2/sql/schema_baseline.sql
git commit -m "feat: add Alembic v2 schema baseline"
```

---

## ⚠️ 注意事項

1. **本番環境では使用しない**: これらは開発ツールです
2. **権限管理は ops/db/ を使用**: ロール作成・権限付与は本番運用スクリプトで
3. **スキーマダンプはレビュー用**: 実行可能なマイグレーションは Alembic で管理

---

**最終更新日**: 2025-12-24
