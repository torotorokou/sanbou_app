 ...
 * 原因: AntD Tabs が高さを伸ばさず、グラフ親の高さが 0 になる
 * 対策: Tabs 連鎖に height:100% / flex を強制する CSS を注入し、Tabs に適用
 *       併せて ChartFrame は親の実寸高さ(px)を計測して渡す
 * =================================================================== */
...
.${TABS_FILL_CLASS} .ant-tabs-tabpane {
  height:100%;
  display:flex;
 ...
 * フォント
 * ========================= */
const FONT = { family: undefined as string | undefined, size: 12 };
...
const nextMonth = (m: IsoMonth): IsoMonth =>
  dayjs(m + "-01").add(1, "month").format("YYYY-MM");
...
const TargetCard: React.FC<{
  ...
  const oneBusinessDay = Math.round(
    (targets.month * (businessWeight / totalW)) || 0
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ color: "#8c8c8c", fontSize: 12, fontWeight: 700 }}>
          目標
        </div>
        <div style={{ color: "#8c8c8c", fontSize: 12, fontWeight: 700 }}>
          実績
        ...
        {(() => {
            ...
            ? sum(
                daily_curve
                  .filter((d) => {
                    ...
                    return (
                      toDate(d.date) >= wstart &&
                      toDate(d.date) <= addDays(wstart, 6)
                    );
                  })
                  .map((d) => d.actual ?? 0)
              )
          ...
          const rowsData = [
            {
              ...
              actual: progress ? progress.mtd_actual : 0,
            },
          ...
          return rowsData.map((r) => {
            ...
            const pctColor =
              ratioRaw >= 1 ? C.ok : ratioRaw >= 0.9 ? C.warn : C.danger;

            return (
              <React.Fragment key={r.key}>
                <div style={{ color: "#595959", fontSize: 12, fontWeight: 800 }}>
                  {r.label}
                </div>
                <div>
                  <Statistic
                    ...
                    suffix="t"
                    valueStyle={{ color: C.primary, fontSize: 18, fontWeight: 800 }}
                  />
                </div>
                <div>
                  <Statistic
                    ...
                    suffix="t"
                    valueStyle={{ color: "#222", fontSize: 18, fontWeight: 800 }}
                  />
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  ...
                  >
                    <Statistic
                      value={pct}
                      suffix="%"
                      valueStyle={{ color: pctColor, fontSize: 12, fontWeight: 700 }}
                    />
                  </div>
                  <Progress
                    percent={barPct}
...
const DailyActualsCard: React.FC<{
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        <ChartFrame style={{ flex: 1, minHeight: 0 }}>
          <BarChart data={chartData} margin={{ left: 0, right: 8, top: 6, bottom: 12 }}>
            ...
            {showPrevMonth && (
              <Line type="monotone" dataKey="prevMonth" stroke="#40a9ff" dot={false} strokeWidth={2} />
            )}
            {showPrevYear && (
              <Line type="monotone" dataKey="prevYear" stroke="#fa8c16" dot={false} strokeWidth={2} />
            )}
  ...
  return (
    <Card bordered size="small" bodyStyle={{ padding: "0 8px 8px", height: "100%", display: "flex", flexDirection: "column", minHeight: 0 }}>
      <Inner />
    </Card>
  );
};
...
const DailyCumulativeCard: React.FC<{
  ...
  const tooltipFormatter = (...args: unknown[]) => {
    ...
    if (payload && typeof payload === "object" && "actualCumulative" in payload) {
      ...
      else if (typeof a === "string" && !Number.isNaN(Number(a)))
        actualCum = Number(a);
    }
    if (v == null || v === "" || Number.isNaN(Number(v))) return ["—", label];
    const valNum = Number(v as unknown as number);
  ...
  const Inner = () => (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
      <Space
        align="baseline"
        style={{ justifyContent: "space-between", width: "100%", paddingBottom: 4 }}
      >
        <Typography.Title level={5} style={{ margin: 0, fontSize: 13 }}>
          日次累積搬入量（累積）
        </Typography.Title>
  ...
  return (
    <Card bordered bodyStyle={{ padding: "0 8px 8px", height: "100%", display: "flex", flexDirection: "column", minHeight: 0 }}>
      <Inner />
    </Card>
  );
};
...
const CombinedDailyCard: React.FC<{
  ...
  return (
    <Card
      ...
      bodyStyle={{ padding: "0 8px 8px", flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}
    >
      <Tabs
        size="small"
...
const CalendarCard: React.FC<{
  ...
  return (
    <Card
      ...
      size="small"
      style={{ height: "100%", display: "flex", flexDirection: "column", ...(style || {}) }}
      bodyStyle={{ display: "flex", flexDirection: "column", padding: 8, gap: 8, flex: 1, minHeight: 0 }}
    >
      ...
      >
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <div style={{ width: 10, height: 10, borderRadius: 3, background: C.ok }} />
          <div style={{ color: "#595959", fontWeight: 700 }}>
            {dayCounts.weekday}
            <span style={{ color: "#8c8c8c", fontWeight: 400, marginLeft: 6 }}>
              ({dayCounts.weekdayRem})
            </span>
          </div>
        </div>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <div style={{ width: 10, height: 10, borderRadius: 3, background: "#ff85c0" }} />
          <div style={{ color: "#595959", fontWeight: 700 }}>
            {dayCounts.sunday}
            <span style={{ color: "#8c8c8c", fontWeight: 400, marginLeft: 6 }}>
              ({dayCounts.sundayRem})
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <div style={{ width: 10, height: 10, borderRadius: 3, background: C.danger }} />
          <div style={{ color: "#595959", fontWeight: 700 }}>
            {dayCounts.secondSunday}
            <span style={{ color: "#8c8c8c", fontWeight: 400, marginLeft: 6 }}>
              ({dayCounts.secondSundayRem})
            </span>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        <CalendarGrid
          ...
          renderCell={(value, inMonth) => {
            ...
            if (!inMonth)
              return <div style={{ color: "#bfbfbf", fontSize: 11 }}>{dayNum}</div>;
            return (
              <div
                style={{
                  width: 22,
...
const ForecastCard: React.FC<{
  ...
  monthTarget: number;
  rows?: DailyCurveDTO[];
}> = ({ forecast, monthTarget, rows }) => {
  useInstallTabsFillCSS();

  const rowsData = rows && rows.length ? rows : [];
  const month = rowsData.length
    ? dayjs(rowsData[0].date).format("YYYY-MM")
    : curMonth();
  const daysInMonth = rowsData.length
    ? rowsData.length
    : dayjs(month + "-01").daysInMonth();
    ...
    for (let idx = 0; idx < rowsData.length; idx++) {
      ...
      const dailyForward =
        i >= forwardStartDay && i <= forwardEndDay
          ? Math.round(
              base * (0.9 + 0.2 * (0.5 + Math.sin((i / daysInMonth) * Math.PI * 2) / 2))
            )
          : 0;
  ...
  const KPIBlock: React.FC<KPIProps> = ({
    title,
    p50,
    p10,
    p90,
    ...
    const pctColor =
      ratio == null ? "#8c8c8c" : ratio >= 1 ? C.ok : ratio >= 0.9 ? C.warn : C.danger;
    return (
      <Card size="small" bodyStyle={{ padding: 8, height: "100%", display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
        <div
          style={{
            height: "100%",
            display: "grid",
          ...
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ fontSize: 11, color: "#8c8c8c" }}>
                レンジ: {p10}–{p90} <span style={{ fontSize: 10 }}>t</span>
              </div>
            </div>
          </div>
  ...
  const WeekLabel: React.FC<{ value?: string; x?: number; y?: number; fill?: string }> = ({
    value = "",
    x = 0,
    y = 0,
    fill = "#000",
    ...
    const padX = 6,
      padY = 4,
      fontSize = 12;
    const estWidth = Math.max(60, String(value).length * fontSize * 0.6);
    const rectX = (x ?? 0) - estWidth / 2 - padX;
    const rectY = (y ?? 0) - fontSize - padY * 2;
    return (
      <g>
        <rect
          x={rectX}
  ...
  return (
    <Card
      ...
      bodyStyle={{ padding: 8, display: "flex", flexDirection: "column", gap: 8, flex: 1, minHeight: 0 }}
    >
        ...
        <Row gutter={[8, 8]} style={{ height: "100%" }}>
          ...
          <Col xs={24} lg={8} style={{ height: "100%" }}>
            <div style={{ height: "100%", display: "grid", gridTemplateRows: "1fr 1fr 1fr", gap: 6 }}>
              <KPIBlock
                title="当日"
                p50={forecast.today.p50}
                p10={forecast.today.p10}
                p90={forecast.today.p90}
                target={monthTarget && rows?.length ? Math.round(monthTarget / rows.length) : null}
              />
              <KPIBlock
                title="今週合計"
                p50={forecast.week.p50}
                p10={forecast.week.p10}
                p90={forecast.week.p90}
                target={monthTarget && rows?.length ? Math.round((monthTarget / rows.length) * 7) : null}
              />
              <KPIBlock
                title="今月末"
                p50={forecast.month_landing.p50}
                p10={forecast.month_landing.p10}
                p90={forecast.month_landing.p90}
                target={monthTarget}
              />
            </div>
          </Col>
                          ...
                          >
                            ...
                            <RTooltip
                              ...
                              formatter={(v: unknown, name: unknown) => {
                                ...
                                const display =
                                  map[key] ?? (key === "label" || key === "undefined" || key === "" ? "" : key);
                                const vs = String(v ?? "");
                      ...
                      <div style={{ flex: 1, minHeight: 0 }}>
                        <ChartFrame style={{ flex: 1, minHeight: 0 }}>
                          <AreaChart data={(() => {
                            ...
                            <ReferenceLine
                              y={forecast.month_landing.p50}
                              stroke={C.primary}
                              strokeDasharray="3 3"
                              strokeWidth={2}
                              label={{
                                value: `月末予測: ${forecast.month_landing.p50}t`,
                                position: "right",
                                fill: C.primary,
                                fontSize: 12,
                              }}
...
const InboundForecastDashboardFull: React.FC = () => {
  ...
  if (loading || !data) {
    return (
      <div style={{ minHeight: "100vh", overflowY: "auto", padding: 12 }}>
        <Row gutter={[8, 8]} style={{ height: "100%" }}>
          <Col span={24}>
  ...
  const maskedRows: DailyCurveDTO[] = data.daily_curve.map((r) => ({
    ...r,
    actual: r.date <= actualCutoff ? r.actual : undefined,
  }));
  const mtdMasked = sum(maskedRows.map((r) => r.actual ?? 0));
  const remainingBizMasked = data.calendar.days.filter(
    (d) => d.date > actualCutoff && d.is_business_day
  ).length;
  const displayedProgress: ProgressDTO = {
    mtd_actual: mtdMasked,
    remaining_business_days: remainingBizMasked,
  };
  ...
  return (
    <div
      style={{
        ...
        overflowY: "auto",
        display: "grid",
        gridTemplateRows: "auto 1fr 1.2fr", // ヘッダー + 上段 + 下段
        rowGap: 8,
        padding: 12,
      ...
      <div>
        <Row justify="space-between" align="middle">
          <Col>
            ...
            <div style={{ color: "#8c8c8c", fontSize: 12 }}>
              月ページ固定 / 週は部分週（日で帰属） / 営業日アライメントで比較
            </div>
          </Col>
          <Col>
            <Space size={8} wrap>
        ...
        <Row gutter={[8, 8]} style={{ height: "100%" }}>
          <Col xs={24} lg={7} style={{ height: "100%" }}>
            <TargetCard
              ...
              daily_curve={maskedRows}
            />
          </Col>
          <Col xs={24} lg={12} style={{ height: "100%" }}>
            <CombinedDailyCard
              rows={maskedRows}
              prevMonthDaily={data.prev_month_daily}
              prevYearDaily={data.prev_year_daily}
            />
          </Col>
        ...
        <Row gutter={[8, 8]} style={{ height: "100%" }}>
          <Col xs={24} style={{ height: "100%" }}>
            <ForecastCard
              ...
              monthTarget={data.targets.month}
              rows={maskedRows}
            />
          </Col>
        </Row>
