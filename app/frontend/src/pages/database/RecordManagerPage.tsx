...
const generateSampleMonth = (month: Dayjs): RecordRow[] => {
  ...
  for (
    ...
    const push = (group: GroupKey, kind: KindKey) => {
      ...
      const status: Status =
        rand() < 0.08 ? "error" : rand() < 0.15 ? "warn" : "ok";
      ...
      rows.push({
        ...
        slip_no:
          group === "manifest"
            ? undefined
            : `SL-${Math.floor(rand() * 900000 + 100000)}`,
        customer: cust,
        weight_kg: weight,
        amount_yen: amount,
        source: "CSV",
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 6 }}>
        {(["shogun_flash", "shogun_final", "manifest"] as GroupKey[]).map(
          (g) => {
...
const RecordManagerPage: React.FC = () => {
  ...
  const deleteRow = (row: RecordRow) => {
    modal.confirm({
      ...
      content: (
        <Text>
          伝票/識別: {row.slip_no || row.id} を削除します。よろしいですか?
        </Text>
      ),
      okText: "削除",
      okButtonProps: { danger: true },
  ...
  const columns: ColumnsType<RecordRow> = [
    {
      ...
      width: 130,
      render: (v: GroupKey) => (
        <Tag color={GROUP_COLOR[v]}>{GROUP_LABEL[v]}</Tag>
      ),
    },
    {
      title: "区分",
        ...
        <Card title="日サマリー">
          <Space direction="vertical">
            ...
            <Space wrap>
              ...
              <Tag color={GROUP_COLOR.shogun_flash}>
                速報 {byGroup.shogun_flash || 0}
              </Tag>
              <Tag color={GROUP_COLOR.shogun_final}>
                最終 {byGroup.shogun_final || 0}
              </Tag>
              <Tag color={GROUP_COLOR.manifest}>
                マニ {byGroup.manifest || 0}
              </Tag>
    ...
    return (
      <Card
        title={
          <Space>
            <Text strong>詳細</Text>
            <Tag color={GROUP_COLOR[selectedRow.group]}>
              {GROUP_LABEL[selectedRow.group]}
            </Tag>
                ...
                <Space direction="vertical">
                  ...
                  <Text>
                    ...
                    {selectedRow.imported_at
                      ? dayjs(selectedRow.imported_at).format(
                          "YYYY/MM/DD HH:mm:ss",
                        )
                      : "-"}
                  </Text>
  ...
  return (
    <div
      style={{
        height: isMobile
          ? "auto"
          : "calc(100dvh - (var(--page-padding, 0px) * 2))",
    ...
    >
      ...
      {isMobile && (
        <div style={{ marginBottom: 8 }}>
          <Select<GroupKey | "all">
            ...
            options={[
              { value: "all", label: "すべて" },
              { value: "shogun_flash", label: "将軍_速報" },
              { value: "shogun_final", label: "将軍_最終" },
              { value: "manifest", label: "マニフェスト" },
            ]}
            style={{ width: "100%" }}
            size="small"
          />
      ...
      {isMobile ? (
        <Tabs
          ...
          onChange={(k) =>
            setMobileActiveTab(k as "calendar" | "list" | "detail")
          }
          items={[
            {
              key: "list",
              label: `一覧 (${filteredRows.length})`,
              children: (
                <div style={{ marginBottom: 8 }}>
                  <Space
                    wrap
                  ...
                  <Table<RecordRow>
                    ...
                    columns={columns.filter((c) =>
                      ["group", "slip_no", "status", "op"].includes(
                        String(c.key ?? ""),
                      ),
                    )}
