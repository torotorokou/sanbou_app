...
export function useSubmitVM() {
  ...
  const doUpload = useCallback(
    async (
      ...
      logger.log("[doUpload] Starting upload...");

      try {
        const response = await repo.upload(filesByType, uploadPath);
        logger.log("[doUpload] Upload successful.", response);
  ...
  const resetUploadState = useCallback(() => {
    logger.log("[resetUploadState] Resetting upload state.");
    setUploadSuccess(false);
    setUploading(false);
  }, []);
