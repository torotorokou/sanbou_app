...
export const CsvPreviewCard: React.FC<Props> = ({
  ...
  const backgroundColor =
    propBackgroundColor || CSV_TYPE_COLORS[type] || "#ffffff";
  ...
  useLayoutEffect(() => {
    const calc = () => {
      ...
      const head = hideHead
        ? null
        : (card.querySelector(".ant-card-head") as HTMLElement | null);
      ...
      const computed = Math.max(
        80,
        Math.floor(cardHeight - headH - padY - HSCROLL - FUDGE),
      );
  ...
  return (
    <div
      ref={cardRef}
      style={{ height: cardHeight, minHeight: 0, overflow: "hidden" }}
    >
      <Card
        size="small"
        title={
          hideHead ? null : (
            <div>
              <span>
                ...
                {isOptional && (
                  <span
                    style={{ color: "#1890ff", marginLeft: 8, fontSize: 13 }}
                  >
                    任意
                  </span>
                )}
