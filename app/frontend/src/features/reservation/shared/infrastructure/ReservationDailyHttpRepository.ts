...
export class ReservationDailyHttpRepository implements ReservationDailyRepository {
  ...
  async upsertManual(payload: ReservationManualInput): Promise<void> {
    console.log(
      "[ReservationDailyHttpRepository] Sending upsertManual request with payload:",
      payload,
    );
    const response = await coreApi.post(
      "/core_api/reservation/manual",
      payload,
    );
    console.log(
      "[ReservationDailyHttpRepository] upsertManual response:",
      response,
    );
  }
