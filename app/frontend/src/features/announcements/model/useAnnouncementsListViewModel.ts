...
export function useAnnouncementsListViewModel(
  ...
  const visibleAnnouncements = useMemo(() => {
    return announcements.filter((ann) => isVisibleForAudience(ann, CURRENT_AUDIENCE));
  }, [announcements]);
        ...
        case 'date-desc':
          ...
          sorted.sort((a, b) => {
            const dateA = new Date(
              visibleAnnouncements.find((ann) => ann.id === a.id)?.publishFrom || 0
            );
            const dateB = new Date(
              visibleAnnouncements.find((ann) => ann.id === b.id)?.publishFrom || 0
            );
  ...
  const importantItems = useMemo<AnnouncementDisplayItem[]>(() => {
    ...
    const filtered =
      selectedTab === 'unread' ? important.filter((item) => item.isUnread) : important;
  ...
  const otherItems = useMemo<AnnouncementDisplayItem[]>(() => {
    ...
    const filtered = selectedTab === 'unread' ? other.filter((item) => item.isUnread) : other;
    return sortItems(filtered);
  }, [displayItems, selectedTab, sortItems]);
