...
export const normalizeRow = (value: unknown): TransportCandidateRow | null => {
  ...
  if (
    typeof entryIdCandidate !== "string" &&
    typeof entryIdCandidate !== "number"
  ) {
    // logger may not be available in some contexts; fallback to console
    console.warn(
      "normalizeRow: invalid entry_id type, skipping row:",
      typeof entryIdCandidate,
      entryIdCandidate,
    );
    return null;
  }
  ...
  return {
    entry_id,
    vendor_code:
      typeof vendorCodeCandidate === "number" ||
      typeof vendorCodeCandidate === "string"
        ? vendorCodeCandidate
        : "",
    vendor_name:
      typeof vendorNameCandidate === "string"
        ? vendorNameCandidate
        : String(vendorNameCandidate ?? ""),
    item_name:
      typeof itemNameCandidate === "string"
        ? itemNameCandidate
        : String(itemNameCandidate ?? ""),
    detail,
    options,
    initial_index,
