/**
 * 受入量予測 - ViewModel Hook
 * Repository経由でデータを取得し、UI用のpropsに整形
 */

import { useEffect, useState } from "react";
import dayjs from "dayjs";
import type { UkeireForecastRepository } from "../repository/UkeireForecastRepository";
import type { IsoMonth, MonthPayloadDTO, DailyCurveDTO } from "../../model/types";
import { sum, getActualCutoffIso, curMonth, monthNameJP, calculateOneBusinessDayTarget, calculateWeekStats, getTodayActual } from "../../services/seriesService";
import type { TargetCardProps } from "@/features/kpiTarget";
...
export const useUkeireForecastVM = (
  ...
  const targetCardProps: TargetCardProps | null = payload
    ? {
        rows: [
          { key: "month", label: "1ヶ月", target: payload.targets.month, actual: mtdMasked },
          {
            key: "week",
            label: "今週",
            target: calculateWeekStats(payload.targets, payload.calendar.days, maskedRows).target,
            actual: calculateWeekStats(payload.targets, payload.calendar.days, maskedRows).actual,
          },
          {
            key: "day",
            label: "1日",
            target: calculateOneBusinessDayTarget(payload.targets, payload.calendar.days),
        ...
        dailyProps: {
          chartData: maskedRows.map((r) => {
            ...
            return {
              ...
              dateFull: r.date,
              prevMonth: payload.prev_month_daily ? payload.prev_month_daily[prevMonthKey] ?? null : null,
              prevYear: payload.prev_year_daily ? payload.prev_year_daily[prevYearKey] ?? null : null,
            };
          }),
        },
        cumulativeProps: {
          cumData: (() => {
            ...
            return maskedRows.map((r) => {
              ...
              const pmKey = dayjs(r.date).subtract(1, "month").format("YYYY-MM-DD");
              const pyKey = dayjs(r.date).subtract(1, "year").format("YYYY-MM-DD");
              const pmVal = payload.prev_month_daily ? payload.prev_month_daily[pmKey] ?? 0 : 0;
              const pyVal = payload.prev_year_daily ? payload.prev_year_daily[pyKey] ?? 0 : 0;
              accPM += pmVal;
              accPY += pyVal;
              return {
                label: dayjs(r.date).format("DD"),
  ...
  const forecastCardProps: ForecastCardProps | null = payload
    ? (() => {
        ...
        const base = payload.targets.month && daysInMonth ? payload.targets.month / daysInMonth : 0;
        const chartData: { label: string; daily: number; dailyForward?: number; actual?: number }[] = [];
        ...
        for (let idx = 0; idx < maskedRows.length; idx++) {
          ...
          const dailyForward =
            i >= forwardStartDay && i <= forwardEndDay
              ? Math.round(base * (0.9 + 0.2 * (0.5 + Math.sin((i / daysInMonth) * Math.PI * 2) / 2)))
              : 0;
          const actualDay = typeof r.actual === "number" ? r.actual : undefined;
          chartData.push({ label: String(i).padStart(2, "0"), daily, dailyForward, actual: actualDay });
        }
        ...
        const oddDayTicks = Array.from({ length: daysInMonth }, (_, i) => String(i + 1).padStart(2, "0")).filter(
          (s) => Number(s) % 2 === 1
        );
        ...
        return {
          kpis: [
            ...
            },
            {
              ...
              target: daysInMonth ? Math.round((payload.targets.month / daysInMonth) * 7) : null,
            },
            {
              title: "今月末",
