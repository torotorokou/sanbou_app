...
import { UPLOAD_CSV_DEFINITIONS, UPLOAD_CSV_TYPES } from '../../dataset-import/model/constants';
...
const getGroupDecl = (def: { group?: string }): string | null =>
  (def?.group || null) as string | null;

/**
 * label→dataset 推定（group が無い場合のフォールバック）
 */
