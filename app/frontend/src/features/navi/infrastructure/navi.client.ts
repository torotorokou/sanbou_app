...
export const NaviApiClient = {
  ...
  async getQuestionOptions(): Promise<QuestionOptionsResponseDto> {
    return await coreApi.get<QuestionOptionsResponseDto>('/core_api/rag/question-options');
  },
   ...
   * core_api経由でrag_apiにフォワード
   */
  async generateAnswer(payload: ChatQuestionRequestDto): Promise<ChatAnswerResponseDto> {
    return await coreApi.post<ChatAnswerResponseDto>('/core_api/rag/generate-answer', payload);
  },
};
