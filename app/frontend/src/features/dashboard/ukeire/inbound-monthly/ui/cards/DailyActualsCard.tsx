...
export const DailyActualsCard: React.FC<DailyActualsCardProps> = ({
  chartData,
  variant = "standalone",
}) => {
  ...
  const businessDayAverage = React.useMemo(() => {
    const businessDays = chartData.filter(
      (d) => d.status === "business" && d.actual != null && d.actual > 0,
    );
  ...
  const prevMonthAverage = React.useMemo(() => {
    const businessDays = chartData.filter(
      (d) => d.status === "business" && d.prevMonth != null && d.prevMonth > 0,
    );
    if (businessDays.length === 0) return null;
  ...
  const prevYearAverage = React.useMemo(() => {
    const businessDays = chartData.filter(
      (d) => d.status === "business" && d.prevYear != null && d.prevYear > 0,
    );
    if (businessDays.length === 0) return null;
    ...
    >
      ...
      >
        ...
        <Space size="small">
          ...
          <Switch
            size="small"
            checked={showBusinessAvg}
            onChange={setShowBusinessAvg}
          />
        </Space>
      </div>
          ...
          >
            ...
            <RTooltip
              ...
              formatter={(...args) => {
                ...
                const p =
                  payloadItem && payloadItem.payload
                    ? payloadItem.payload
                    : null;
              ...
              content={(props) => {
                ...
                const weekday =
                  ...
                  "dateFull" in payload
                    ? ["日", "月", "火", "水", "木", "金", "土"][
                        dayjs(
                          String((payload as { dateFull?: string }).dateFull),
                        ).day()
                      ]
                    : null;
                const labelText =
                  props.label != null
                    ? `${props.label}日${weekday ? ` (${weekday})` : ""}`
                    : "";
                  ...
                  if (item.value != null && !Number.isNaN(Number(item.value))) {
                    ...
                    if (
                      ...
                      if (actualVal !== 0 && !Number.isNaN(actualVal)) {
                        const diffPct =
                          ((valNum - actualVal) / actualVal) * 100;
                        const sign = diffPct >= 0 ? "+" : "";
            ...
            <Bar dataKey="actual">
              {chartData.map((entry, idx) => (
                <Cell
                  key={`cell-${idx}`}
                  fill={colorForDate(entry.dateFull, entry.status)}
                />
              ))}
            </Bar>
            {showPrevMonth && (
