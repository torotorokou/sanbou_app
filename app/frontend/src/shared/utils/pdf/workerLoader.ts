...
export async function ensurePdfJsWorkerLoaded(): Promise<void> {
  ...
  const alreadySet = Boolean(
    pdfjs.GlobalWorkerOptions.workerSrc && typeof pdfjs.GlobalWorkerOptions.workerSrc === 'string'
  );
  if (alreadySet) return;

  // Use env override if provided; else point to CDN to avoid bundling
  const CDN_WORKER_URL = 'https://cdn.jsdelivr.net/npm/pdfjs-dist@5.3.93/build/pdf.worker.min.mjs';
  const WORKER_URL = (import.meta as ImportMeta & { env: Record<string, string | undefined> }).env.VITE_PDFJS_WORKER_URL || CDN_WORKER_URL;

  pdfjs.GlobalWorkerOptions.workerSrc = WORKER_URL;
  // Ensure workerPort is not set so pdf.js will use workerSrc
  (pdfjs.GlobalWorkerOptions as unknown as { workerPort?: Worker | null }).workerPort = null;
}
