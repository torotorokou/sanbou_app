...
function readANT(projectRoot: string): AntMap {
  ...
  if (!("md" in map) || !("lg" in map) || !("xl" in map)) {
    throw new Error(
      "[vite-plugin-custom-media] bp must include at least md, lg, and xl.",
    );
  }
  return map;
}
...
function writeOut(projectRoot: string, css: string): string {
  const outPath = path.resolve(projectRoot, "src/app/styles/custom-media.css");
...
export default function customMediaPlugin(): Plugin {
  ...
  const regenerate = () => {
    ...
    console.log(
      `[vite-plugin-custom-media] generated: ${path.relative(projectRoot, out)}`,
    );
  };
