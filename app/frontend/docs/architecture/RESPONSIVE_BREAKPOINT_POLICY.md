...
```typescript
Mobile:  ≤ 767px   (0 〜 767)
Tablet:  768-1279px (768 〜 1279)
Desktop: ≥ 1280px  (1280 〜 ∞)
```

### 重要: Tablet の定義

**Tablet = 768-1279px**（1024-1279px を必ず含む）

- タブレット端末だけでなく、小型ノートPCも含む
- 「Laptop」という運用判定は作らない（混乱の元）
- 1024-1279px が漏れることで発生するサイドバー不具合を根絶

### 視覚的な境界

```
画面幅:    0     640    768    1024    1280    1920
          │      │      │       │       │       │
...
export const isTablet = (w: number) => w >= BP.tabletMin && w < BP.desktopMin;
...
export type ResponsiveFlags = {
  ...
  isTablet: boolean;   // 768-1279（重要: 1024-1279を含む）
...
- **フォント**: 標準（15-16px）
- **間隔**: 標準（12-16px）
- **対象デバイス**: iPad, 小型ノートPC, 狭いウィンドウ

**重要**: 1024-1279px もこのカテゴリに含まれる
