...
- **提供API**: 
  - `bp` (ANT互換値: xs:0, sm:576, md:768, lg:992, xl:1200)
  - `mq` (メディアクエリヘルパー)
...
5. **新bp値体系への移行検討**
   - 現在: `bp.md=768, bp.xl=1200` (ANT互換・実運用)
   - 将来: `bp.sm=640, bp.md=768, bp.lg=1024, bp.xl=1280` (標準的)
   - ⚠️ **注意**: 全ページのUI検証が必須（サイドバー、グリッド、カード等）
...
function MyComponent() {
  ...
  return (
    <div>
      {isNarrow ? <MobileView /> : <DesktopView />}
    </div>
  );
}
