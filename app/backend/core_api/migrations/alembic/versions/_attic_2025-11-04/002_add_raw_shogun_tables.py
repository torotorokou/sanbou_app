...
def upgrade() -> None:
    ...
    for csv_type in generator.get_csv_types():
        ...
        for col in columns_def:
            col_type = getattr(sa, col['type'])() if col['type'] in ['String', 'Integer', 'Numeric', 'Boolean'] else sa.Date()
            columns.append(
                sa.Column(
                    col['en_name'],
                    col_type,
        ...
        # 共通カラムを追加
        columns.extend([
            sa.Column('raw_data_json', postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment='元データJSON'),
            sa.Column('uploaded_at', sa.TIMESTAMP(), nullable=False, server_default=sa.text('NOW()'), comment='アップロード日時'),
            sa.Column('created_at', sa.TIMESTAMP(), nullable=False, server_default=sa.text('NOW()'), comment='作成日時'),
