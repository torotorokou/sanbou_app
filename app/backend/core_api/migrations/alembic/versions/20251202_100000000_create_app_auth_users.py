...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("id", sa.BigInteger(), autoincrement=True, nullable=False, 
                 comment="ユーザーID（主キー）"),
        sa.Column("auth_provider", sa.Text(), nullable=False, server_default="google", 
                 comment="認証プロバイダ（google, iap など）"),
        sa.Column("auth_subject", sa.Text(), nullable=False, 
                 comment="認証プロバイダ内のユーザー識別子（sub claim）"),
        sa.Column("email", sa.Text(), nullable=False, 
                 comment="メールアドレス（ログイン用）"),
        sa.Column("display_name", sa.Text(), nullable=False, 
                 comment="表示名"),
        sa.Column("roles", postgresql.ARRAY(sa.Text()), nullable=False, server_default=sa.text("'{}'"), 
