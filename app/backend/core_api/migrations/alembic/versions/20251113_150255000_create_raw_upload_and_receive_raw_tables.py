...
def upgrade():
    ...
    if not _table_exists("raw", "upload_file"):
        op.create_table(
            ...
            sa.Column("csv_type", sa.String(20), nullable=False, comment="receive / yard / shipment"),
            sa.Column("file_size_bytes", sa.BigInteger(), nullable=True, comment="ファイルサイズ (bytes)"),
            sa.Column("row_count", sa.Integer(), nullable=True, comment="データ行数（ヘッダー除く）"),
            sa.Column("uploaded_at", sa.DateTime(timezone=True), server_default=sa.text("CURRENT_TIMESTAMP"), nullable=False),
            sa.Column("uploaded_by", sa.String(100), nullable=True, comment="アップロードユーザー"),
