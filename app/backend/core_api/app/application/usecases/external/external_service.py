...
DEPRECATED: このServiceは非推奨です。
  - 代わりに app/application/usecases/external/external_api_uc.py の各UseCase を使用してください
    * AskRAGUseCase: RAG質問
    * ListManualsUseCase: マニュアル一覧取得
    * GetManualUseCase: 特定マニュアル取得
    * GenerateReportUseCase: レポート生成
    * ClassifyTextUseCase: テキスト分類
  - Router層では app/config/di_providers.py の各プロバイダー を Depends で注入してください
