...
class ShogunCsvRepository:
    ...
    def save_csv_by_type(self, csv_type: str, df: pd.DataFrame) -> int:
        ...
        logger.info(f"[DEBUG REPO] Has upload_file_id={('upload_file_id' in df_renamed.columns)}, Has source_row_no={('source_row_no' in df_renamed.columns)}")
        ...
        if schema == "raw":
            ...
            logger.info(f"[DEBUG REPO] [raw] Saving {csv_type} with ALL columns ({len(df_to_save.columns)} cols): {list(df_to_save.columns)[:10]}")
        else:
            ...
            logger.info(f"[DEBUG REPO] [stg] YAML valid columns for {csv_type}: {len(valid_columns)} cols")
            ...
            logger.info(f"[DEBUG REPO] [stg] Before filter: {len(df_renamed.columns)} cols, After filter will keep: {len(valid_columns_with_tracking)} cols")
            df_to_save = filter_defined_columns(df_renamed, valid_columns_with_tracking, log_dropped=True)
            logger.info(f"[DEBUG REPO] [stg] After filter: {len(df_to_save.columns)} cols: {list(df_to_save.columns)[:15]}")
            
            # SQL 保存可能な型に正規化（pandas特有の型をPython標準型に変換）
            df_to_save = to_sql_ready_df(df_to_save)
            logger.info(f"[DEBUG REPO] [stg] After to_sql_ready: {len(df_to_save.columns)} cols: {list(df_to_save.columns)[:15]}")
        
        model_class = create_shogun_model_class(csv_type, table_name=table_name, schema=schema)
        
        # [DEBUG] モデルクラスのカラム定義を確認
        model_columns = [c.name for c in model_class.__table__.columns]
        logger.info(f"[DEBUG REPO] [{schema}] Model has {len(model_columns)} columns: {model_columns[:15]}")
        ...
        for idx, record in enumerate(records):
            ...
            if idx == 0:
                logger.info(f"[DEBUG REPO] First payload keys ({len(payload)} keys): {list(payload.keys())[:15]}")
                logger.info(f"[DEBUG REPO] First payload sample: {payload}")
                # [DEBUG] モデルに存在しないキーをチェック
                payload_keys = set(payload.keys())
                model_keys = set(model_columns) - {'id', 'created_at'}  # id, created_at は自動生成
                missing_in_model = payload_keys - model_keys
                missing_in_payload = model_keys - payload_keys
                if missing_in_model:
                    logger.warning(f"[DEBUG REPO] Payload has keys not in model: {missing_in_model}")
                if missing_in_payload:
                    logger.warning(f"[DEBUG REPO] Model has columns not in payload: {missing_in_payload}")
        ...
        try:
            ...
            if 'slip_date' in df_final.columns:
                ...
                if before_len != after_len:
                    logger.info(f"[DEBUG REPO] Filtered out {before_len - after_len} empty rows (slip_date=None)")
            
            logger.info(f"[DEBUG REPO] Final DataFrame columns before to_sql: {list(df_final.columns)[:15]}")
            ...
            if schema == 'raw':
                ...
                logger.info(f"[DEBUG REPO] Using TEXT dtype for raw layer (except INTEGER columns: {integer_columns})")
