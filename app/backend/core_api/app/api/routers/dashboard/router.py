...
@router.get(
    ...
    summary="Get target and actual metrics",
)
def get_target_metrics(
    date: date_type = Query(
        ...,
        description="Target date (YYYY-MM-DD). For monthly view, use first day of month.",
    ),
    mode: Literal["daily", "monthly"] = Query(
        "monthly",
        description="Fetch mode: 'daily' for specific date, 'monthly' for month-level view with day/week masking",
    ),
    ...
    **Usage Examples:**
    ...
    if not output.found or output.data is None:
        ...
        raise NotFoundError(resource_type="Target card data", identifier=f"{date} (mode={mode})")
