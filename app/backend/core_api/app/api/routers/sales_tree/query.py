...
@router.post(
    ...
    summary="Get sales tree summary",
)
def get_summary(
    req: SummaryRequest,
    ...
    **使用例:**
    ...
    try:
        logger.info(
            f"POST /analytics/sales-tree/summary: mode={req.mode}, date_from={req.date_from}, date_to={req.date_to}"
        )
...
@router.post(
    ...
    response_model_by_alias=True,
    summary="Get daily series data",
)
def get_daily_series(
    req: DailySeriesRequest,
    uc: FetchSalesTreeDailySeriesUseCase = Depends(
        get_fetch_sales_tree_daily_series_uc
    ),
):
    """
    ...
    **使用例:**
    ...
    try:
        logger.info(
            f"POST /analytics/sales-tree/daily-series: date_from={req.date_from}, date_to={req.date_to}"
        )
...
@router.post(
    ...
    response_model_by_alias=True,
    summary="Get pivot data for drill-down",
)
def get_pivot(
    req: PivotRequest,
    ...
    **ページネーション:**
    ...
    try:
        logger.info(
            f"POST /analytics/sales-tree/pivot: base={req.base_axis}:{req.base_id}, target={req.target_axis}"
        )
