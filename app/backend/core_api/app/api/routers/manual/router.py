    ...
    try:
        async with httpx.AsyncClient(timeout=timeout) as client:
            async with client.stream("GET", upstream, headers=req_headers) as resp:
                ...
                logger.info(
                    f"[BFF Manual] Streaming doc: {filename}, status={resp.status_code}, content-type={resp.headers.get('content-type')}"
                )
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            logger.info(
                ...
                extra=create_log_context(
                    operation="proxy_manual_categories", status_code=r.status_code
                ),
            )
            return r.json()
...
async def proxy_manual_sections(manual_id: str):
    ...
    try:
        ...
        logger.error(
            f"[BFF Manual] Manual sections error: {e.response.status_code}",
            exc_info=True,
        )
        raise ExternalServiceError(
            service_name="manual_api",
...
async def proxy_manual_assets(file_path: str, request: Request):
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout) as client:
            ...
            logger.info(
                f"[BFF Manual] Asset retrieved: {file_path}, status={resp.status_code}, content-type={resp.headers.get('content-type')}"
            )
        ...
        raise ExternalServiceError(
            service_name="manual_api",
            message=f"Asset retrieval failed: {str(e)}",
            cause=e,
            status_code=502,
        )
