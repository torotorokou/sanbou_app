# /backend/migrations_v2/alembic/env.py
# ------------------------------------------------------------
# Alembic v2 環境設定（ベースライン起点の新管理）
# - DSN: POSTGRES_* 環境変数から動的構築（backend_shared.infra.db.url_builder 使用）
# - フォールバック: DB_DSN または DATABASE_URL 環境変数
# - 複数スキーマ（raw/ref/mart/stg）差分検出: include_schemas=True
# - 型/デフォルト値の差分検出: compare_type / compare_server_default
# - プロジェクト直下 (/backend) を sys.path に追加して import 解決
#
# ★ v2 と legacy の違い:
#   - script_location が /backend/migrations_v2/alembic （このファイル）
#   - version_table は引き続き public.alembic_version を使用（同一DB内で管理）
#   - baseline revision (0001_baseline) を起点とする
# ------------------------------------------------------------
...
# このファイルの位置: /backend/migrations_v2/alembic/env.py
# parents[2] = /backend
# ------------------------------------------------------------
