import pandas as pd
import numpy as np
from app.api.st_app.utils.cleaners import enforce_dtypes, strip_whitespace
...
def processor_func(dfs: dict) -> pd.DataFrame:
    ...
    """
    # 出荷一覧のdfを取得
    shipment_df = make_csv(dfs)

    # マスターCSVの読込
    master_csv = process1(shipment_df)

    # df_shipmentからデータを取得
    df_after = process2(master_csv, shipment_df)
...
def make_csv(dfs):
    # 各データフレームを取得
    shipment_df = dfs.get("shipment", pd.DataFrame())

    # 必要なカラムのリストを取得
    cul_list = get_required_shipment_columns()

    # 必要なカラムが存在するか確認
    missing_columns = [col for col in cul_list if col not in shipment_df.columns]
    if missing_columns:
        raise ValueError(
    ...
    # 必要なカラムのみに絞る
    shipment_df = shipment_df[cul_list]

    # 空白除去
    shipment_df = strip_whitespace(shipment_df)

    # テンプレートに基づく型定義を取得
    expected_dtypes = get_expected_dtypes_by_template("balance_management_table")
    shipment_dtypes = expected_dtypes.get("shipment", {})

    # データ型を適用
    if shipment_dtypes:
        shipment_df = enforce_dtypes(shipment_df, shipment_dtypes)
...
def process2(master_df: pd.DataFrame, shipment_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in master_df.iterrows():
        condition = shipment_df[columns_to_match[0]] == row[columns_to_match[0]]
        for col in columns_to_match[1:]:
            val = row[col]
            if pd.notna(val) and str(val) not in ["0", "nan", "NaN"]:
                condition &= shipment_df[col] == val

        filtered = shipment_df[condition].copy()
...
def get_required_shipment_columns() -> list:
    """
    ...
    # shipmentのカラムを取得（存在しない場合は空のリストを返す）
    shipment_cols = required_cols.get("shipment", [])

    return shipment_cols
...
# from app.api.st_app.components.custom_button import centered_download_button
