...
from app.api.st_app.utils.config_loader import get_csv_label_map
from app.api.st_app.utils.logger import app_logger  # ロガーのインポート
...
def handle_uploaded_files(required_keys):
    ...
    Parameters:
    - required_keys: アップロードが必要なファイルのキー一覧（例：["receive", "shipment"]）
    ...
    # アップロード対象のキー（例: "receive", "shipment"）ごとに確認
    for key in required_keys:
        ...
        if uploaded:
            # このキーに期待されるCSV種別名を取得（例: "shipment" → "出荷データ"）
