# backend/app/st_app/logic/manage/block_unit_price_interactive_initial.py
# -*- coding: utf-8 -*-
...
def execute_initial_step(df_formatted: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    try:
        ...
        # 同一パッケージ内モジュールは循環インポートを避けるため遅延インポート
        from . import block_unit_price_interactive_main as main_mod

        session_id = make_session_id()

        # テンプレート設定の取得
        template_key = "block_unit_price"
        template_config = main_mod.get_template_config()[template_key]
        template_name = template_config["key"]
        csv_keys = template_config["required_files"]

        # マスターCSVの読み込み
        config = main_mod.get_template_config()["block_unit_price"]
        master_path = config["master_csv_path"]["vendor_code"]
        master_csv = main_mod.load_master_and_template(master_path)

        # データフレームの読み込み
        df_dict = main_mod.load_all_filtered_dataframes(
        ...
        # 運搬費用データの読み込み
        mainpath = main_mod.MainPath()
        reader = main_mod.ReadTransportDiscount(mainpath)
        df_transport_cost = reader.load_discounted_df()

        # 前処理
        df_shipment = make_df_shipment_after_use(master_csv, df_shipment)
        df_shipment = main_mod.apply_unit_price_addition(master_csv, df_shipment)
        df_shipment = main_mod.apply_transport_fee_by1(df_shipment, df_transport_cost)
