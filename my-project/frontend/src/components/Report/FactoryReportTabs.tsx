// /app/src/components/Report/FactoryReportTabs.tsx
...
interface FactoryReportTabsProps {
    selectedReport: FactoryReportKey;
    onChangeReport: (reportKey: FactoryReportKey) => void;
    currentStep: number;
    title: string;
}
 ...
 * - タイトル表示
 */
const FactoryReportTabs: React.FC<FactoryReportTabsProps> = ({
    selectedReport,
    onChangeReport,
    currentStep,
    title,
    ...
    return (
        <div style={{ marginBottom: 16 }}>
            ...
            <div style={{
                ...
                {Object.entries(FACTORY_REPORT_KEYS).map(([key, config]) => (
                    <button
                        key={key}
                        onClick={() => onChangeReport(key as FactoryReportKey)}
                        style={{
                            display: 'flex',
                            alignItems: 'center',
                            gap: 4,
...
export default FactoryReportTabs;
