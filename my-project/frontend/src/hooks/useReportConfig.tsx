...
export const useReportConfig = (configPackage: ReportConfigPackage) => {
    const config = useMemo(() => {
        return {
            ...
            // API設定
            getApiUrl: (reportKey: string) => configPackage.apiUrlMap[reportKey],
            
            // CSV設定
            getCsvConfig: (reportKey: string) => configPackage.csvConfigMap[reportKey],
            
            // PDF関連
            generatePdf: (reportKey: string) => configPackage.pdfGeneratorMap[reportKey],
            getPreviewImage: (reportKey: string) => configPackage.pdfPreviewMap[reportKey],
