...
from app.api.st_app.logic.manage.processors.block_unit_price.process1 import (
...
# デバッグ用
# from app.api.st_app.utils.debug_tools import debug_pause
...
def process(dfs):
    ...
    # --- 段階的処理の実行 ---
    if mini_step == 0:
        _process_step0(df_shipment, master_csv, df_transport_cost)
        return None
    elif mini_step == 1:
        _process_step1(df_transport_cost)
        return None
    elif mini_step == 2:
        return _process_step2(df_shipment, master_csv, df_transport_cost)

    return None


def _process_step0(
    df_shipment: pd.DataFrame, master_csv: pd.DataFrame, df_transport_cost: pd.DataFrame
) -> None:
    """繰り返し不必要な処理
    ...
    st.session_state.df_shipment_first = df_shipment
...
def _process_step1(df_transport: pd.DataFrame) -> None:
    ...
    df_after = st.session_state.df_shipment_first
    ...
    if not st.session_state.get("block_unit_price_confirmed", False):
        ...
        st.session_state.df_shipment = df_after
        st.rerun()
    else:
        # 選択済みの場合：次のステップへ進む
...
def _process_step2(
    df_shipment: pd.DataFrame, master_csv: pd.DataFrame, df_transport: pd.DataFrame
) -> pd.DataFrame:
    """最終計算処理を行うステップ2の実装
    ...
    df_after = st.session_state.df_shipment
    ...
    master_csv = make_sum_date(master_csv, df_shipment)
