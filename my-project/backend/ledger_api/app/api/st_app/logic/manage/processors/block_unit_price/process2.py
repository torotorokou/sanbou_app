...
def apply_transport_fee_by_vendor(
    ...
    """
    from app.api.st_app.logic.manage.utils.column_utils import (
...
def add_date_information(df: pd.DataFrame, df_shipment: pd.DataFrame) -> pd.DataFrame:
    """日付情報を追加する関数

    Args:
        df: セルマッピング情報を含むデータフレーム
        df_shipment: 出荷データフレーム
    ...
    # 伝票日付を令和形式に変換
    reiwa_date = to_reiwa_format(df_shipment["伝票日付"].iloc[0])
