...
def process(shipment_df: pd.DataFrame, master_csv: pd.DataFrame = None) -> pd.DataFrame:
    """
    shipment_dfから balance_management_table.csv の条件に基づいて集計を行う

    Args:
        shipment_df (pd.DataFrame): 出荷データ
    ...
    """
    # 必要なカラムの取得
    required_columns = get_required_shipment_columns()

    # 必要なカラムが存在するか確認
    missing_columns = [
        col for col in required_columns if col not in shipment_df.columns
    ]
    if missing_columns:
        raise ValueError(
    ...
    for _, row in master_df.iterrows():
        # 条件に基づくフィルタリング
        mask = pd.Series(True, index=shipment_df.index)

        # 各カラムの条件を適用
        for col in columns_to_check:
            if pd.notna(row[col]) and col in shipment_df.columns:
                mask &= shipment_df[col].astype(str) == str(row[col])

        # フィルタリングされたデータを取得
        filtered_df = shipment_df[mask]
