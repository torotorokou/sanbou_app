# Alembic v2: ベースライン起点の新マイグレーション管理
...
- ✅ **Legacy 退避**: 既存Alembic は `migrations_legacy/` に保存（参照用）
- ✅ **新規管理**: `migrations_v2/` で今後の変更を管理
...
└─────────────────────────────────────────────────────────────┘
                          ...
                          ↓
┌─────────────────────────────────────────────────────────────┐
│ Legacy Alembic (migrations_legacy/)                          │
│ - 過去の履歴として保存                                        │
│ - 通常運用では使用しない                                      │
│ - make al-up-env-legacy で参照可能                           │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ Alembic v2 (migrations_v2/)                                  │
...
│   │       └── ... (100+ files)
│
├── migrations_legacy/             # Legacy Alembic（参照用）
│   ├── alembic.ini
│   └── alembic/
│       ├── env.py
│       └── versions/
