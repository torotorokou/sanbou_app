...
CalendarCardUkeire (wrapper)
  ↓
CalendarCard (ukeire専用)
  ↓ useUkeireCalendarVM
  ↓   ↓ decorateCalendarCells
  ↓   ↓
  ↓ CalendarRepositoryForUkeire (HTTP)
  ↓
UkeireCalendar (表示コンポーネント)
  ↓
CalendarCore (共通カレンダーコア)
```
...
6. **表示コンポーネント** (`UkeireCalendar`)
   - ペイロードを受け取り
   - `CalendarCore`でレンダリング
   - 当日ハイライトを適用
...
- `UkeireCalendar` - 受入専用の表示コンポーネント

**設計方針**: 共通部分は`features/calendar`に、受入固有のロジックは`features/dashboard/ukeire`に配置。
